...
m_evaluation.py
===============
Canonical evaluation utilities for the ML framework.  Two responsibilities,
one home:

1. **Metric primitives** (sklearn-based, mirroring ``ml_5_model_evaluation``).
   ``ml_5`` is the leading authority for metric computation; these wrappers call
   the *exact same* sklearn functions so values are identical, and every other
   layer (the cross-method comparison below, the read-model, the report) draws
   from this single source instead of re-deriving metrics.  MAPE is a FRACTION
   (``0.05 == 5%``), matching ``ml_5`` and ``m_sector_quality``.

2. **Cross-method comparison engine** — methodology-aware comparison of point
   and probabilistic forecasts across model families (Pipeline reducers,
   AutoETS, STL+ETS, Chronos-Bolt, baseline).  These families were not all run
   on the same evaluation contract, so the engine:
     (a) normalises every model to one canonical schema,
     (b) aligns on the intersection of (sector, target_date, horizon),
     (c) computes point / regime / per-sector / per-horizon metrics, PI
         calibration and CRPS where supported,
     (d) runs pairwise Diebold-Mariano + Friedman/Nemenyi tests and builds a
         multi-criteria scorecard + decision matrix.

In-framework usage (preferred): ``ml_orchestrator.run_comparison`` loads every
family straight from the eval DB via
``m_pipeline_loader.load_families_from_eval_db`` and calls ``compare_all_models``.

Legacy notebook usage: ``model_comparison.ipynb`` still reads per-method
``cv_output/*.parquet`` via the ``load_*_predictions`` readers below — those are
retained for backward compatibility and are not used by the framework path.
"""
...
# ============================================================================
# METRIC PRIMITIVES — single source, sklearn-based, mirroring ml_5
# ============================================================================
# These wrap the identical sklearn calls ml_5_model_evaluation uses, so adopting
# them anywhere (incl. ml_5 itself) leaves logged values byte-identical.  MAPE
# and directional accuracy are returned as FRACTIONS for repo-wide consistency
...
def rmse(y_true, y_pred) -> float:
    """Root mean squared error (sklearn)."""
    return float(np.sqrt(mean_squared_error(y_true, y_pred)))


def mape(y_true, y_pred) -> float:
    """Mean absolute percentage error as a FRACTION (0.05 == 5%), via sklearn."""
    return float(mean_absolute_percentage_error(y_true, y_pred))


def r2(y_true, y_pred) -> float:
    """R² (sklearn).  Returns NaN for <2 samples, where R² is undefined."""
...
def bias(y_true, y_pred) -> float:
    """Mean signed error (y_pred − y_true)."""
    y_true = np.asarray(y_true, dtype=float)
...
def directional_accuracy(y_true, y_pred) -> float:
    """Fraction of steps whose direction of change is predicted correctly."""
    y_true = np.asarray(y_true, dtype=float)
    y_pred = np.asarray(y_pred, dtype=float)
    if len(y_true) < 2:
        return float("nan")
...
def seasonal_naive_mae(y_train, sp: int = 4) -> float:
    ...
    Copied verbatim from ``ml_5_model_evaluation._seasonal_naive_mae`` so the two
    stay identical — ml_5 remains the authority.
    """
...
def mase(y_true, y_pred, y_train, sp: int = 4) -> float:
    """Mean absolute scaled error: MAE / seasonal-naive MAE on the train window.

    Mirrors the MASE computation in ``ml_5`` (the gate metric): finite only when
    the scaler is finite and > 0, else NaN.
    """
    scaler = seasonal_naive_mae(y_train, sp=sp)
    m = mae(y_true, y_pred)
    if math.isfinite(m) and math.isfinite(scaler) and scaler > 0:
        return float(m / scaler)
    return float("nan")


def point_metrics(df: pd.DataFrame, group_by: Optional[list[str]] = None) -> pd.DataFrame:
    """Aggregate point-forecast metrics over a tidy predictions frame.

    Each metric is computed (via the primitives above) on rows with non-NaN
    ``y_pred``/``y_true``.  Columns: ``n, MAE, RMSE, MAPE, R2, bias, dir_acc``.
    Groups with no usable rows contribute n=0 and NaN metrics.
    """
    if group_by is None:
        group_by = ["model_name"]
    ...
    def _agg(g):
        ...
        return pd.Series({
            ...
            "R2":      r2(yt, yp),
            "bias":    bias(yt, yp),
            "dir_acc": directional_accuracy(yt, yp),
        })
...
# ----------------------------------------------------------------------------
# Canonical schema (cross-method comparison)
# ----------------------------------------------------------------------------
...
CANONICAL_COLS = [
    "model_name",      # str — e.g. "Pipeline", "AutoETS_Stat"
...
# ============================================================================
# LOADERS — per-method parquet readers (LEGACY: model_comparison.ipynb only)
# ============================================================================
# The in-framework comparison reads the eval DB via
# m_pipeline_loader.load_families_from_eval_db.  The readers below remain for
# the standalone notebook that compares against cv_output/*.parquet.

def load_autoets_predictions(
    canonical_parquet: str | Path,
) -> pd.DataFrame:
    """
    Load AutoETS predictions from run_autoets_cv.py output.
...
def load_stl_ets_predictions(
    ...
    """
    Load STL+ETS predictions from run_stl_ets_cv.py output.
...
def load_chronos_predictions(
    ...
    """
    Load Chronos-Bolt walk-forward predictions from run_chronos_cv.py output.
...
# ============================================================================
# METRICS — point-forecast accuracy (group aggregations over aligned data)
# ============================================================================
# compute_point_metrics is an alias for the canonical point_metrics aggregator
# (defined in the primitives section) so the comparison code below reads the
# same way it always did while drawing metrics from the single source.

compute_point_metrics = point_metrics
...
def compute_regime_metrics(
    ...
    """
    Pre- vs post-regime metrics.  Default split at 2023-01-01.
    """
    df = df.copy()
